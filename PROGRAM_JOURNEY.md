...
| Pair approved | One proposal and approval covers the match and [introduction](emails/match-introduction.html). The Chair later approves the [training calendar invitation](emails/training-invitation.html) with its date and guests. | Google records RSVPs. Chair handles conflicts, questions and attendance arrangements. Neither acceptance nor an email reply itself marks training complete. Changes and cancellation keep the same calendar event. |
...
| Mentorship underway | Separate [month 3](emails/check-in-3.html), [month 6](emails/check-in-6.html) and [month 9](emails/check-in-9.html) check-ins ask each person about meetings, value and contact needs. Month 3 counts from the first meeting; later check-ins count from the previous check-in. | Clear answers are recorded and classified. Questions, unclear answers and support requests go to the Chair. Feedback does not itself end the relationship. |
