...
The participant Privacy Notice describes these services by purpose. Maintainers must preserve the actual provider details here: Google Workspace also holds training invitations and guest responses; OpenAI supports routine processing and the Chair's Codex session. Verify no-training settings separately for both AI routes. Before accepting real participant information, verify that API data-sharing settings and the Chair's model-improvement setting are disabled for your installation. Automated calls use `store: false`, which does not remove provider abuse-monitoring retention.
