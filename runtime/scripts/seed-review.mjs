...
import {sha256} from '../applications.mjs';
...
const token=crypto.randomUUID()+crypto.randomUUID(),pairId=crypto.randomUUID();
...
queries.push(`INSERT INTO requests(id,pair_id,application_id,role,kind,period,scheduled_for,sent_at,deadline,token_hash) VALUES(${[crypto.randomUUID(),pairId,ids.mentee,'mentee','final'].map(quote).join(',')},12,${[now,now,deadline,await sha256(token)].map(quote).join(',')})`);
...
console.log(JSON.stringify({url:'http://127.0.0.1:8787/check-in#'+token,fixtureOnly:true}));
