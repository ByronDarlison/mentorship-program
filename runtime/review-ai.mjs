...
export function createReviewAI({apiKey,spendingVerified=false,fixturesOnly=true,db,fetcher=fetch}={}){
  ...
  return {
    async interpretReply(text,request,plannedDate,identity){
      ...
      const answerSchema=object({meetings:{type:['integer','null']},progress:{type:['string','null']},value:{type:['string','null']},returnInterest:{type:['string','null']},recommendations:{type:['string','null'],description:'Exact suggestions for improving the program, only when recommendations is an allowed field. Otherwise null. Do not confuse suggestions with the value the participant received.'},contact:{type:['boolean','null']}});
