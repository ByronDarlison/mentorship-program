...
test('month 12 recommendations are accepted for both roles without adding outcome or completion requirements',()=>{
  for(const role of ['mentee','mentor']){
    ...
    const request={kind:'final',period:12,role,answers,classifications:{value:'Meaningful',progress:'Meaningful',returnInterest:'Interested'},deadline:'2026-12-31',superseded:0};
    ...
    assert.throws(()=>validateFeedback({...request,period:0},{recommendations:'More practice.'}));
  }
});
